 ...
 *   - minSeverity: the DISPLAY threshold in σ above the patient's baseline (the backend
 *     filters anomalies to severity >= minSeverity). Range 2σ–6σ, default 3σ. Shown to the
 *     user as a 0–100% scale (nobody thinks in σ): higher % = stricter = fewer, stronger
 *     anomalies. The mapping is pct = 50 + (σ−2)·10, so 2σ→50%, 3σ→60%, … 6σ→90%.
 *     Slider steps 1σ (= 10%).
 */
...
interface SeverityInferenceContextValue {
  ...
  minSeverity: number;               // σ — pass to the read endpoint; also drives the slider
