import type { Patient, GlucoseReading, TimeInRange, AnomalyDetection } from "@/types"; // Demo data for development
...
export function generateDemoReadings(): GlucoseReading[] {
    ...
    for (let i = 288; i >= 0; i--) {
        ...
        glucose += (6.1 - glucose) * 0.02 + (Math.random() - 0.5) * 0.33 + mealEffect;
        glucose = Math.max(2.8, Math.min(19.4, glucose));  // 50–350 mg/dL in mmol/L

        const status: GlucoseReading["status"] =
            glucose < 3.0 ? "very_low" :
                glucose < 3.9 ? "low" :
                    glucose <= 10.0 ? "in_range" :
                        glucose <= 13.9 ? "high" : "very_high";
