...
def load_pooled(ohio_root: Path = OHIO_ROOT):
    """Return ({pid: patient}, hupa_pids, ohio_pids). Ohio = train-split (12 unique)."""
    hu = load_hupa_cohort()
    oh = load_ohio_cohort(ohio_root, year="both", split="train")   # 12 unique, longest records
    cohort = {p.pid: p for p in hu}
    for p in oh:
        assert p.pid not in cohort, f"pid collision HUPA/Ohio: {p.pid}"
        cohort[p.pid] = p
    return cohort, sorted(p.pid for p in hu), sorted(p.pid for p in oh)


def pooled_split(hupa_pids, ohio_pids, seed: int = 42,
                 n_hupa_test: int = 7, n_ohio_test: int = 4, n_val: int = 6) -> dict:
    ...
    hu, oh = shuf(hupa_pids), shuf(ohio_pids)
    hupa_test, ohio_test = hu[:n_hupa_test], oh[:n_ohio_test]
    tv = shuf(hu[n_hupa_test:] + oh[n_ohio_test:])
    return {"train": tv[n_val:], "val": tv[:n_val], "test": hupa_test + ohio_test,
            "hupa_test": hupa_test, "ohio_test": ohio_test}
