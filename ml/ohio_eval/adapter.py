...
def _attr(e: ET.Element, key: str) -> str:
    """A required XML attribute as str (these OhioT1DM fields are always present)."""
    v = e.get(key)
    if v is None:
        raise ValueError(f"<{e.tag}> is missing the '{key}' attribute")
    return v
...
def load_ohio_patient(path: Path) -> OhioPatient:
    ...
    times = [_ts(_attr(e, "ts")) for e in glu]
    ...
    g_val = np.array([float(_attr(e, "value")) / MGDL_PER_MMOL for e in glu], dtype=np.float64)
    ...
    basal = sorted(((_ts(_attr(e, "ts")), float(_attr(e, "value"))) for e in _events(root, "basal")),
                   key=lambda x: x[0])
    ...
    for ev in _events(root, "temp_basal"):
        s, e = max(0, minute(_ts(_attr(ev, "ts_begin")))), min(T, minute(_ts(_attr(ev, "ts_end"))) + 1)
        if s < e:
            basal_rate[s:e] = float(_attr(ev, "value"))
    basal_mU_min = (basal_rate / 60.0 * 1000.0).astype(np.float32)

    # -- boluses (units + announced carbs) ------------------------------------
    boluses: list[Bolus] = []
    for ev in _events(root, "bolus"):
        m = minute(_ts(_attr(ev, "ts_begin")))
        if 0 <= m < T:
            carb_g = float(ev.get("bwz_carb_input") or 0.0)
            boluses.append(Bolus(minute=m, units=float(_attr(ev, "dose")), carb_g=carb_g))

    # -- meals (logged carbs, independent of boluses) -------------------------
    meals: list[Meal] = []
    for ev in _events(root, "meal"):
        m = minute(_ts(_attr(ev, "ts")))
        if 0 <= m < T:
            meals.append(Meal(minute=m, carb_g=float(_attr(ev, "carbs"))))
