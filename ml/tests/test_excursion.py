"""Unit tests for glucose-derived label placement. See ml/docs/EXCURSION_LABELS.md."""
import sys
...
def test_subthreshold_jitter_is_not_onset():
    ...
    g = _trace() + rng.normal(0, 0.3, 600)   # σ 0.3 << rise 1.1
    assert excursion_window(g, 100, "missed") is None
...
def test_never_returns_is_capped():
    ...
    s, e = excursion_window(g, meal, "missed")
...
def test_class_cap_differs():
    ...
    g = _trace(); g[100:] = 12.0
    _, e_missed = excursion_window(g, 100, "missed")
    _, e_large = excursion_window(g, 100, "large")
...
def test_dropout_inside_rise_breaks_run():
    ...
    s, e = excursion_window(g, 100, "missed")
